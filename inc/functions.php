...
function p_my_sklad_import_single_product($ms_product)
{
	try {
		...
		// 2. Если не найден, поиск по SKU (externalCode)
		if (!$product_id && !empty($externalCode)) {
			$args = [
				'post_type' => 'product',
				'post_status' => ['publish'],
				'meta_query' => [
		...
		// 3. Если не найден, поиск по названию (name)
		if (!$product_id && !empty($name)) {
			$args = [
				'post_type' => 'product',
				'post_status' => ['publish'],
				'title' => $name,  // Фильтр по пост_title
				'posts_per_page' => 1,
			];
			$query = new WP_Query($args);
			$product_id = $query->have_posts() ? $query->posts[0]->ID : 0;
