...
function p_my_sklad_import_single_product($ms_product, $settings)
{
    try {
        ...
        if (isset($packs) && count($packs) > 0) {
            // Собрать опции (имена, уникальные)
            $option_names = [];
            foreach ($packs as $pack) {
                if (isset($pack['quantity']) && !empty($pack['quantity'])) {
                    $option_names[] = trim($pack['quantity']); // E.g., '1 кг'
                }
            }
            $option_names = array_unique($option_names);

            // Если опций нет, не трогаем атрибуты/вариации
            if (empty($option_names)) {
                p_my_sklad_log()->debug('Нет опций для пакетов — пропуск атрибутов и вариаций', ['packs' => $packs]);
            } else {
                // Логика для переключения на вариативный продукт (ваш код, оставлен без изменений)
                if (!$product->is_type('variable')) {
                    ...
                    $new_product->set_image_id($product->get_image_id());
                    $new_product->set_gallery_image_ids($product->get_gallery_image_ids());
                    $new_product->set_category_ids($product->get_category_ids());
                    $new_product_id = $new_product->save();
                    if ($new_product_id) {
                        $product->delete();
                        $product = wc_get_product($new_product_id);
                        p_my_sklad_log()->debug('Продукт переключен на вариативный', ['new_product_id' => $new_product_id]);
                    } else {
                        throw new Exception('Не удалось создать вариативный продукт');
                    }
                }

                // Работа с атрибутом 'Вес'
                $attribute_name = 'ves';
                $attribute_id = 0;
                ...
                foreach ($option_names as $option_name) {
                    $term_exists = term_exists($option_name, $taxonomy);
                    if (!$term_exists) {
                        $insert_result = wp_insert_term($option_name, $taxonomy);
                        if (is_wp_error($insert_result)) {
                            p_my_sklad_log()->error('Ошибка терминирования опции', [
                    ...
                    if ($term && !is_wp_error($term)) {
                        ...
                        p_my_sklad_log()->error('Не удалось найти термин для опции', ['option_name' => $option_name]);
                    }
                }
                ...
                foreach ($existing_variations as $var_id) {
                    ...
                    if ($variation && $variation->is_type('variation')) {
                        ...
                        if (isset($attrs[$taxonomy])) {
                            $existing_variation_attrs[$attrs[$taxonomy]] = $variation;
                        }
                    }
                }

                // Для каждого терма: если вариации нет — создаём, иначе оставляем
                foreach ($terms as $term) {
                    $slug = $term->slug;
                    if (!isset($existing_variation_attrs[$slug])) {
                ...
                // Скрываем старые вариации, чьи атрибуты больше не актуальны
                $active_slugs = array_column($terms, 'slug');
                foreach ($existing_variation_attrs as $slug => $variation) {
